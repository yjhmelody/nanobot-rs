...
impl FileWatcher {
    ...
    async fn refresh(&self) -> AgentResult<()> {
        ...
        // Detect changes - read old entries without holding lock across comparison
        let old_entries = self.inner.entries.read().clone();
        let is_initial = old_entries.is_empty();

        let changed_names: HashSet<String> = if !is_initial {
            new_entries
                .iter()
        ...
        // Update entries
        *self.inner.entries.write() = Arc::new(new_entries.clone());
...
mod tests {
    ...
    #[tokio::test]
    async fn watched_files_cache_event_hooks_triggered() {
        let temp = tempfile::tempdir().expect("tempdir");
        let root = temp.path().to_path_buf();
